...
describe('AuthService', () => {
          ...
          useValue: {
            ...
            findUserById: jest
              ...
              .mockImplementation((id: string) =>
                UserMockData.users.find((item) => item.id == id),
              ),
            findUserByEmail: jest
              .fn()
              .mockImplementation((email: string) =>
                UserMockData.users.find((item) => item.email == email),
              ),
  ...
  it('Sign In with no existing email throws error', async () => {
    jest
      ...
      .mockImplementation((pass1: string, pass2: string) => pass1 == pass2);
    await expect(authService.signIn('wrong', 'idontcare')).rejects.toThrow(
      AuthenticationError,
    );
  });
  ...
  it('Sign In with wrong password throws error', async () => {
    jest
      .spyOn(bcrypt, 'compare')
      .mockImplementation((pass1, pass2) => pass1 == pass2);
    await expect(
      authService.signIn(UserMockData.users[0].email, 'wrong'),
  ...
  it('Validate with no existing email throws error', async () => {
    jest
      ...
      .mockImplementation((pass1: string, pass2: string) => pass1 == pass2);
    await expect(
      authService.validateUser('wrong', 'idontcare'),
    ).resolves.toBeNull();
  });
  ...
  it('Validate with wrong password throws error', async () => {
    jest
      ...
      .mockImplementation((pass1, pass2) => pass1 == pass2);
    await expect(
      authService.validateUser(UserMockData.users[0].email, 'wrong'),
    ).rejects.toThrow(AuthenticationError);
  });
